...
if __name__ == '__main__':
    ...
    parser.add_argument('--log', type=str, default='results_attack/tmp')
    ...
    edge_index = np.load("./attack_data/%s_0.20.npy"%args.dataset)
    dataset.data.edge_index = torch.LongTensor(edge_index)
    x = np.load("./attack_data/%s_0.20_x.npy"%args.dataset)
    dataset.data.x = torch.Tensor(x)
    num_nodes = x.shape[0]
    num_features = x.shape[1]
    y = np.load("./attack_data/%s_0.20_y.npy"%args.dataset)
    dataset.data.y = torch.LongTensor(y)
